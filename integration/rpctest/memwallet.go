...
type memWallet struct {
	...
	addrs map[uint32]bglutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]bglutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*bglutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          bglutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (bglutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []bglutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt bglutil.Amount,
	feeRate bglutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := bglutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate bglutil.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate bglutil.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt bglutil.Amount
	for _, output := range outputs {
		outputAmt += bglutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() bglutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance bglutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (bglutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := bglutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
