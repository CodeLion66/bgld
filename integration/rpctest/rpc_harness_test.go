...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt bglutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(bglutil.Amount(5 * bglutil.SatoshiPerBitcoin))
	...
	txid = genSpend(bglutil.Amount(500 * bglutil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(bglutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*bglutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, bglutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := bglutil.Amount(250 * bglutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = bglutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := bglutil.Amount(50 * bglutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := bglutil.Amount(numMatureOutputs * 50 * bglutil.SatoshiPerBitcoin)
