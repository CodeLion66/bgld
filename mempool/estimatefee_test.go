...
func (eft *estimateFeeTester) testTx(fee bglutil.Amount) *TxDesc {
	eft.version++
	return &TxDesc{
		TxDesc: mining.TxDesc{
			Tx: bglutil.NewTx(&wire.MsgTx{
				Version: eft.version,
			}),
			Height: eft.height,
			Fee:    int64(fee),
...
func (eft *estimateFeeTester) newBlock(txs []*wire.MsgTx) {
	...
	block := bglutil.NewBlock(&wire.MsgBlock{
		Transactions: txs,
	})
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	...
	for i := uint32(0); i < txPerRound; i++ {
		newTx := eft.testTx(bglutil.Amount(rand.Intn(1000000)))
