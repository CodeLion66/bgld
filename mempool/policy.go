...
const (
	...
	DefaultMinRelayTxFee = bglutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee bglutil.Amount) int64 {
	...
	if minFee < 0 || minFee > bglutil.MaxSatoshi {
		minFee = bglutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *bglutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee bglutil.Amount) bool {
...
func checkTransactionStandard(tx *bglutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee bglutil.Amount,
...
func GetTxVirtualSize(tx *bglutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
