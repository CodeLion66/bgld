...
const (
	...
	MinHighPriority = bglutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr bglutil.Address) (*bglutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *bglutil.Tx, height int32) error {
...
func logSkippedDeps(tx *bglutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress bglutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*bglutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := bglutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := bglutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A bglutil.Block should use saved in the state to avoid
