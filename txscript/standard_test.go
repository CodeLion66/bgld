...
// newAddressPubKey returns a new bglutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) bglutil.Address {
	addr, err := bglutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new bglutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) bglutil.Address {
	addr, err := bglutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new bglutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) bglutil.Address {
	addr, err := bglutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the bglutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the bglutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := bglutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := bglutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := bglutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := bglutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := bglutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*bglutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*bglutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*bglutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*bglutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*bglutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
